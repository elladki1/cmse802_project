...
Run from command line:
    python -m src.report --data_dir ./data/raw_data --output ./results/report --top 20
"""
...
def prompt_if_missing(args):
    ...
    if not args.output:
        args.output = input("Enter path to output directory (e.g., ./results/report/): ").strip() 
...
def main():
    ...
    wts = mol_wts(df)
    outfile = os.path.join(args.output, f"wts_dist.png")
    plt.hist(wts, bins=50, edgecolor='black', alpha=0.7)
    plt.title("Molecular Weights Distribution")
    plt.xlabel("Molecular Weights (g/mol)")
